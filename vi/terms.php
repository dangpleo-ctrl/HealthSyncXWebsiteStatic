<?php
$page_title = "Điều khoản & Điều kiện";
$meta_description = "Điều khoản & Điều kiện của HealthSyncX - Các điều khoản pháp lý điều chỉnh việc sử dụng trang web và dịch vụ của chúng tôi.";
$canonical_url = "/vi/terms";
$en_page = "terms.php";
include '../includes/header-vi.php';
?>

<!-- Hero -->
<section class="bg-gradient-to-br from-primary/5 via-background to-primary/10 py-20">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        <h1 class="text-4xl md:text-5xl font-bold text-foreground mb-4">
            Điều khoản & Điều kiện
        </h1>
        <p class="text-lg md:text-xl text-muted-foreground max-w-3xl mx-auto">
            Các điều khoản pháp lý điều chỉnh việc sử dụng dịch vụ của chúng tôi
        </p>
    </div>
</section>

<!-- Terms Content -->
<section class="py-20">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="bg-card border border-card-border rounded-lg p-8 md:p-12">
            <div class="prose prose-lg max-w-none">
                <p class="text-sm text-muted-foreground italic mb-8">
                    Cập nhật lần cuối: <?php echo date('j F, Y'); ?>
                </p>
                
                <h2 class="text-2xl font-bold text-foreground mb-4">Chấp Nhận Điều Khoản</h2>
                <p class="text-muted-foreground leading-relaxed mb-6">
                    Bằng việc truy cập hoặc sử dụng trang web HealthSyncX, bạn đồng ý tuân thủ các điều khoản và điều kiện này.
                </p>
                
                <h2 class="text-2xl font-bold text-foreground mb-4">Sử Dụng Trang Web</h2>
                <p class="text-muted-foreground leading-relaxed mb-6">
                    Người dùng không được sử dụng trang web này cho các mục đích bất hợp pháp, tấn công mạng hoặc trích xuất dữ liệu trái phép.
                </p>
                
                <h2 class="text-2xl font-bold text-foreground mb-4">Quyền Sở Hữu Trí Tuệ</h2>
                <p class="text-muted-foreground leading-relaxed mb-6">
                    Tất cả nội dung, biểu trưng và tài liệu trên trang web này thuộc sở hữu của Công ty TNHH HealthSyncX. Nghiêm cấm sao chép hoặc phân phối khi chưa được sự đồng ý.
                </p>
                
                <h2 class="text-2xl font-bold text-foreground mb-4">Miễn Trách Nhiệm</h2>
                <p class="text-muted-foreground leading-relaxed mb-6">
                    Thông tin trên trang web này chỉ nhằm mục đích cung cấp kiến thức chung và không được xem là tư vấn chuyên môn hoặc y tế.
                </p>
                
                <h2 class="text-2xl font-bold text-foreground mb-4">Liên Kết Tới Trang Khác</h2>
                <p class="text-muted-foreground leading-relaxed mb-6">
                    Trang web của chúng tôi có thể chứa liên kết đến các trang web của bên thứ ba. Chúng tôi không chịu trách nhiệm về nội dung hoặc chính sách bảo mật của họ.
                </p>
                
                <h2 class="text-2xl font-bold text-foreground mb-4">Luật Điều Chỉnh</h2>
                <p class="text-muted-foreground leading-relaxed mb-6">
                    Các điều khoản này được điều chỉnh bởi luật pháp của nước Cộng hòa Xã hội Chủ nghĩa Việt Nam. Đối với các hợp tác tại New Zealand, luật pháp địa phương cũng sẽ được tuân thủ khi cần thiết.
                </p>
                
                <h2 class="text-2xl font-bold text-foreground mb-4">Thông Tin Liên Hệ</h2>
                <p class="text-muted-foreground leading-relaxed mb-4">
                    Nếu bạn có câu hỏi về các Điều khoản & Điều kiện này, vui lòng liên hệ với chúng tôi:
                </p>
                <ul class="list-none text-muted-foreground leading-relaxed space-y-2">
                    <li><strong>Email:</strong> <a href="mailto:contact@healthsyncx.org" class="text-primary hover:underline">contact@healthsyncx.org</a></li>
                    <li><strong>Công ty:</strong> Công ty TNHH HealthSyncX</li>
                    <li><strong>Địa điểm:</strong> Việt Nam & New Zealand</li>
                </ul>
            </div>
        </div>
    </div>
</section>

<?php include '../includes/footer-vi.php'; ?>
